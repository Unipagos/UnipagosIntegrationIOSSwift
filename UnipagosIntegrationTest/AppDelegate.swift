...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, open url: URL, sourceApplication sourceApp: String?, annotation theAnnotation: Any) -> Bool {
        
        let query = url.query! as NSString;
        let queryComponents = NSMutableDictionary();
        let components = query.components(separatedBy:"&");
        
        for component in components {
            var keyValuePairArray = component.components(separatedBy:"=");
            ...
            var results = queryComponents.value(forKey: key) as! NSMutableArray?; // URL spec says that multiple values are allowed per key
            if results == nil{
